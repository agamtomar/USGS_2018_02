...
PROGRAM MAIN
    ...
    INTEGER :: nnum=1, threshold=10, left, right, nbuffer, mtag, offset
    INTEGER, ALLOCATABLE :: buffer(:)
    INTEGER :: irqs(2)
    INTEGER :: mstat(MPI_STATUS_SIZE, 2)
    INTEGER :: my_min, my_max, dnum
    ...
    ! 1.) each process calculates sequence lengths in range my_rank*100:(my_rank+1)*100
...
CONTAINS
    ...
    SUBROUTINE COMPUTE_LENGTHS(m,n,lengths,icount, threshold)
        ! Computes the Collatz sequence length for every number in the range [m,n]
        ! Numbers whose sequence-lengths are greater thanstored in
        IMPLICIT NONE
        INTEGER, INTENT(InOut) :: lengths(:), icount
        INTEGER,
        INTEGER :: i, length
    ...
    END SUBROUTINE COMPUTE_LENGTHS
