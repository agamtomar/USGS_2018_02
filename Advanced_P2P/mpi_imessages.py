...
if (rank == (size-1)):
    ...
    # ISend and Irecv are METHODS of the MPI Communicator (which we call cw)
    # They RETURN a Request object whose status we can query to check if the 
    # exchange has completed
    sreq = cw.Isend(svals,dest=mydest,tag=mytag )
    ...
    rreq = cw.Irecv(rvals,source=mysource,tag=yourtag)

    rreq.Waitall([sreq,rreq])
...
if (rank == 0):
    ...
    rreq = cw.Irecv(rvals,source=mysource,tag=yourtag) # Receive from rank size-1
